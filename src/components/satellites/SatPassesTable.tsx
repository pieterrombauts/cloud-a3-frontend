...
const SatPassesTable: React.FC<SatPassesTableProps> = (props) => {
  ...
  useEffect(() => {
    ...
    let combined = [...radiorows, ...visualrows].sort((a,b) => dateSort(a.startTime,b.startTime));
    setRows(combined);
  },[radio, visual])
  useEffect(() => {
    if (userLoc !== null) {
      ...
      setLoading(true);
      axios.get(`https://cors-anywhere.herokuapp.com/https://api.n2yo.com/rest/v1/satellite/radiopasses/${props.satellite.noradID}/${lat}/${lng}/0/7/60/&apiKey=86G9FS-PZTG9E-AH6DUE-4Q1M`).then((response) => {
        if (response.data.passes !== undefined || response.data.passes !== null) {
          let radiopasses = response.data.passes?.map((point: SatPassType) => ({
            type: "Radio",
      ...
      axios.get(`https://cors-anywhere.herokuapp.com/https://api.n2yo.com/rest/v1/satellite/visualpasses/${props.satellite.noradID}/${lat}/${lng}/0/7/60/&apiKey=86G9FS-PZTG9E-AH6DUE-4Q1M`).then((response) => {
        if (response.data.passes !== undefined || response.data.passes !== null) {
          let visualpasses = response.data.passes?.map((point: SatPassType) => ({
            type: "Visual",
            direction: point.startAzCompass + " - " + point.endAzCompass,
            startTime: DateTime.fromSeconds(point.startUTC),
            endTime: DateTime.fromSeconds(point.endUTC),
            ...
            <TableBody>
              {rows.map((row: SatDataType) => (
                <TableRow>
                  ...
                  <TableCell>{row.startTime.toFormat("MMM dd, HH:mm:ss")}</TableCell>
                  <TableCell>{row.endTime.toFormat("MMM dd, HH:mm:ss")}</TableCell>
                  <TableCell>{row.totalTime + " s"}</TableCell>
                </TableRow>
              ))}
              {rows.length === 0 && props.satellite.noradID !== "" &&
                <TableRow>
                  <TableCell colSpan={5}>There are no passes in the next week for this satellite.</TableCell>
                </TableRow>
              }
              {props.satellite.noradID === "" &&
                <TableRow>
                  <TableCell colSpan={5}>Search for a satellite to see results</TableCell>
                </TableRow>
              }
            </TableBody>
          </Table>
