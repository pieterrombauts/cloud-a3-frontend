...
const SatelliteGlobe: React.FC<SatelliteGlobeProps> = (props) => {
  ...
  useEffect(() => {
    if (props.noradID !== "") {
      ...
      let endTime = DateTime.utc().plus({days: 1}).toFormat("yyyy-MM-dd_HH:mm:ss");
      axios.get(`https://cors-anywhere.herokuapp.com/http://www.orbitalpredictor.com/api/predict_orbit/?sats=${props.noradID}&start=${startTime}&end=${endTime}&format=czml&type=orbit`).then((response) => {
