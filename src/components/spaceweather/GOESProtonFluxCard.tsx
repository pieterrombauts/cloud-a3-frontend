...
const dataURL = `${NOAA_BASE_URL}/json/goes/primary/integral-protons-plot-6-hour.json`;
      ...
      if (Array.isArray(dataItems) && dataItems.length > 0) {
        let newData: ChartData = {
          labels: dataItems
            ...
            .filter(
              (value: Date, index: number, self: Date[]) =>
                self.findIndex((date) => date.getTime() === value.getTime()) ===
                index,
            ),
          datasets: [
            {
              label: '>= 10 MeV',
              data: dataItems
                .filter((dataItem: any) => dataItem.energy === '>=10 MeV')
                .map((dataItem: any) => dataItem.flux),
            ...
            },
            {
              label: '>= 50 MeV',
              data: dataItems
                .filter((dataItem: any) => dataItem.energy === '>=50 MeV')
                .map((dataItem: any) => dataItem.flux),
            ...
            },
            {
              label: '>= 100 MeV',
              data: dataItems
                .filter((dataItem: any) => dataItem.energy === '>=100 MeV')
                .map((dataItem: any) => dataItem.flux),
